...
Avvia in background il thread [[ssh_file_reader]] per aggiornare i dati supplementari ogni `59` secondi.
Riprende l'acquisizione dall'ultimo timestamp salvato tramite [[timestamp_manager]].
...
>	Controlla la dimensione della coda. Se non è piena, scrive il file `.h5` tramite [[netcdf4_h5_manager]].

> *def main*() -> None:
>	Crea `SAVE_PATH`, avvia il thread [[ssh_file_reader]], legge l'ultimo timestamp da [[timestamp_manager]] e avvia il loop di acquisizione. Per ogni pacchetto valido: reshape dei dati → `write_packet` → aggiornamento timestamp. Se i dati non sono nuovi, dorme 150ms per evitare busy-loop. Termina con `sys_exit()` se `COUNT != 1`.
